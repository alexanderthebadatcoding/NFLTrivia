...
def load_college_lookup(verbose=True):
    """gsis_id -> college_name, from the single static nflreadpy.load_players() table
    (this is a one-row-per-player biographical table, unlike load_rosters() which is
    season-by-season and has spotty college data for older seasons)."""
    global _players_cache
    if _players_cache is None:
        if verbose:
            print("  loading player bios via nflreadpy.load_players() for college lookup...", file=sys.stderr)
...
def load_rookie_season_lookup(verbose=True):
    """gsis_id -> rookie_season, from the same static nflreadpy.load_players() bio table
    used for college lookups."""
    global _players_cache
    if _players_cache is None:
        load_college_lookup(verbose=verbose)  # populates _players_cache as a side effect
    players = nfl.load_players()
    return dict(zip(players["gsis_id"].to_list(), players["rookie_season"].to_list()))


def compile_rookie_top(year, top_n=None, verbose=True):
    """
    Best rookies for a single season, grouped by position and ranked by PPR
    fantasy points. A player counts as a rookie for `year` when their
    rookie_season (from nflreadpy.load_players()) equals `year` -- this is
    nflverse's own determination of rookie status, not just "first year we
    have stats for them" (which can be wrong for players who sat a year hurt,
    practice-squad seasons, etc).
    """
    result = {"QB": [], "RB": [], "WR": [], "TE": []}
    stats = load_season_stats(year, verbose=verbose)
    rookie_lookup = load_rookie_season_lookup(verbose=verbose)

    id_col = "player_id" if "player_id" in stats.columns else "gsis_id"
    team_col = "recent_team" if "recent_team" in stats.columns else "team"
    yy = str(year)[-2:]

    rookies = stats.filter(
        pl.col(id_col).map_elements(lambda pid: rookie_lookup.get(pid) == year, return_dtype=pl.Boolean)
    )
    if "fantasy_points_ppr" in rookies.columns:
        rookies = rookies.sort("fantasy_points_ppr", descending=True)
    ...
    for row in rookies.iter_rows(named=True):
        ...
        if verbose:
            pts = row.get("fantasy_points_ppr")
            pts_str = f" [{pts:.1f} pts]" if pts is not None else ""
            print(f"    + [{group}] {entry['name']} ({entry['team']}): {entry['stats']}{pts_str}", file=sys.stderr)
        result[group].append(entry)
...
def write_rookie_year_file(year, data, out_dir="data"):
    import os
    os.makedirs(out_dir, exist_ok=True)
    fname = f"Rookie{year}.json"
    out_path = os.path.join(out_dir, fname)
    with open(out_path, "w") as f:
        json.dump(data, f, indent=2)
    counts = {k: len(v) for k, v in data.items()}
...
def main():
    ...
    parser.add_argument("year", type=str, nargs="?", default=None,
                         help="Season year (e.g. 2025), or 'all' to compile every season in --start..--end. "
                              "Not required when using --colleges by itself.")
    ...
    parser.add_argument("--top-n-only", action="store_true",
                         help="Skip per-team '{TEAM}{YEAR}.json' files entirely. Use with --top-n and/or "
                              "--rookies to only write those leaguewide/rookie files.")
    parser.add_argument("--colleges", type=str, default=None,
                         help="Comma-separated college names (as nflverse spells them, e.g. "
                              ...
                              "Independent of --teams/--divisions/--top-n; ignores the positional 'year' "
                              "argument other than requiring one be given.")
    ...
    parser.add_argument("--rookies", action="store_true",
                         help="Also write a '{YEAR}Rookie.json'-style file (actually 'Rookie{YEAR}.json') "
                              "per requested year: that season's rookies (per nflreadpy's rookie_season "
                              "field), grouped by position and ranked by PPR fantasy points.")
    parser.add_argument("--rookie-top-n", type=int, default=None, metavar="N",
                         help="Cap each position's rookie list to the top N by PPR fantasy points. "
                              "Default: no cap, include every rookie with non-zero stats that season.")
    args = parser.parse_args()

    if args.top_n_only and not (args.top_n or args.rookies):
        parser.error("--top-n-only skips per-team files, so pair it with --top-n and/or --rookies.")

    if args.year is None and not args.colleges:
        parser.error("Must specify a year (or 'all'), unless using --colleges.")
    ...
    if not teams and not (args.top_n or args.rookies or args.top_n_only):
        parser.error("Must specify --teams and/or --divisions (or --divisions all), "
                      "unless using --top-n/--top-n-only or --rookies.")
    ...
    for year in years:
        ...
        if args.rookies:
            print(f"  -- {year} rookies --", file=sys.stderr)
            rookie_data = compile_rookie_top(year, top_n=args.rookie_top_n, verbose=verbose)
            write_rookie_year_file(year, rookie_data, out_dir=args.out_dir)
